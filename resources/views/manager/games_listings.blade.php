                ...
                <div class="card">
                    ...
                    <div class="card-body text-center">
                        ...
                        <div class="d-flex align-items-center justify-content-center">
                            <!-- Offline Button -->
                            <button title="Offline" class="btn btn-danger open-modal"
                                data-game-id="{{ $game->id }}" 
                                data-game-title="{{ $game->title }}" 
                                data-stock="{{ $game->$offline_stock }}" 
                                data-price="{{ $game->{"ps{$n}_offline_price"} }}" 
                                data-type="offline"
                                data-platform="{{ $n }}">
                                {!! $up !!}{{ $game->$offline_stock }} 
                                <span>&nbsp;|&nbsp;</span> 
                                {!! $moneyIcon !!}{{ $game->{"ps{$n}_offline_price"} }}
                            </button>
                        </div>

                        <div class="d-flex align-items-center justify-content-center mt-2">
                            <!-- Primary Button -->
                            <button title="Primary" class="btn btn-success me-2 open-modal"
                                data-game-id="{{ $game->id }}" 
                                data-game-title="{{ $game->title }}" 
                                data-stock="{{ $game->$primary_stock }}" 
                                data-price="{{ $game->{"ps{$n}_primary_price"} }}" 
                                data-type="primary"
                                data-platform="{{ $n }}">
                                {!! $up !!}{{ $game->$primary_stock }} 
                                <span>&nbsp;|&nbsp;</span> 
                                {!! $moneyIcon !!}{{ $game->{"ps{$n}_primary_price"} }}
                            </button>

                            <!-- Secondary Button -->
                            <button title="Secondary" class="btn btn-warning open-modal"
                                data-game-id="{{ $game->id }}" 
                                data-game-title="{{ $game->title }}" 
                                data-stock="{{ $game->$secondary_stock }}" 
                                data-price="{{ $game->{"ps{$n}_secondary_price"} }}" 
                                data-type="secondary"
                                data-platform="{{ $n }}">
                                {!! $up !!}{{ $game->$secondary_stock }} 
                                <span>&nbsp;|&nbsp;</span> 
                                {!! $moneyIcon !!}{{ $game->{"ps{$n}_secondary_price"} }}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-body">
                <form id="sellForm">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="form-control" placeholder="Enter Client Phone" name="phone" required>
                    </div>
                    <div class="form-group mb-3">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control" name="store_profile_id" id="store_profile_id" required>
                    ...
                    <input type="hidden" name="stock" id="game_stock">
                    <input type="hidden" name="platform" id="game_platform">
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="submit" class="btn btn-primary" form="sellForm">Submit</button>
            </div>
        </div>
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.open-modal').forEach(function(button) {
            button.addEventListener('click', function() {
                var gameTitle = this.dataset.gameTitle;
                var gameId = this.dataset.gameId;
                var stock = this.dataset.stock;
                var price = this.dataset.price;
                var type = this.dataset.type;
                var platform = this.dataset.platform; // Dynamic platform (ps4, ps5, etc.)

                // Set the modal title and hidden fields
                document.getElementById('gameModalLabel').innerText = `${gameTitle} (${type} PS${platform})`;
                document.getElementById('game_id').value = gameId;
                document.getElementById('game_stock').value = stock;
